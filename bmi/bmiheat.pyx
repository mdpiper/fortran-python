...
cdef class Heat:
    ...
    def initialize(self, config_file):
        status = <int>initialize(self._bmi, to_bytes(config_file),
                                     len(config_file))
        ok_or_raise(status)

    def finalize(self):
        status = <int>finalize(self._bmi)
        ok_or_raise(status)

    cpdef object get_component_name(self):
        self.reset_str_buffer()
        ok_or_raise(<int>get_component_name(self._bmi,
                                           self.STR_BUFFER,
                                           MAX_COMPONENT_NAME))
    ...
    cpdef int get_input_var_name_count(self):
        cdef int count = 0
        ok_or_raise(<int>get_input_var_name_count(self._bmi, &count))
        return count
    ...
    cpdef object get_input_var_names(self):
        ...
        try:
            ...
            ok_or_raise(<int>get_input_var_names(self._bmi, names, count))
    ...
    cpdef int get_output_var_name_count(self):
        cdef int count = 0
        ok_or_raise(<int>get_output_var_name_count(self._bmi, &count))
        return count
    ...
    cpdef object get_output_var_names(self):
        ...
        try:
            ...
            ok_or_raise(<int>get_output_var_names(self._bmi, names, count))
    ...
    cpdef double get_start_time(self):
        cdef double time
        ok_or_raise(<int>get_start_time(self._bmi, &time))
        return time

    cpdef double get_end_time(self):
        cdef double time
        ok_or_raise(<int>get_end_time(self._bmi, &time))
        return time

    cpdef double get_current_time(self):
        cdef double time
        ok_or_raise(<int>get_current_time(self._bmi, &time))
        return time

    cpdef double get_time_step(self):
        cdef double step
        ok_or_raise(<int>get_time_step(self._bmi, &step))
        return step

    cpdef object get_time_units(self):
        self.reset_str_buffer()
        ok_or_raise(<int>get_time_units(self._bmi, self.STR_BUFFER,
                                       MAX_UNITS_NAME))
        return to_string(self.STR_BUFFER)

    cpdef update(self):
        status = <int>update(self._bmi)
        ok_or_raise(status)

    cpdef update_frac(self, time_frac):
        status = <int>update_frac(self._bmi, time_frac)
        ok_or_raise(status)

    cpdef update_until(self, time_later):
        status = <int>update_until(self._bmi, time_later)
        ok_or_raise(status)

    cpdef int get_var_grid(self, var_name):
        cdef int grid_id
        ok_or_raise(<int>get_var_grid(self._bmi,
                                          to_bytes(var_name),
    ...
    cpdef object get_grid_type(self, grid_id):
        self.reset_str_buffer()
        ok_or_raise(<int>get_grid_type(self._bmi, grid_id,
                                       self.STR_BUFFER,
                                       MAX_TYPE_NAME))
        return to_string(self.STR_BUFFER)
    ...
    cpdef int get_grid_rank(self, grid_id):
        cdef int rank
        ok_or_raise(<int>get_grid_rank(self._bmi, grid_id, &rank))
        return rank

    cpdef int get_grid_size(self, grid_id):
        cdef int size
        ok_or_raise(<int>get_grid_size(self._bmi, grid_id, &size))
        return size
    ...
    cpdef np.ndarray get_grid_shape(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_shape(self._bmi, grid_id,
                                            &shape[0], rank))
        return shape
    ...
    cpdef np.ndarray get_grid_spacing(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_spacing(self._bmi, grid_id,
                                              &spacing[0], rank))
        return spacing
    ...
    cpdef np.ndarray get_grid_origin(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_origin(self._bmi, grid_id,
                                             &origin[0], rank))
        return origin
    ...
    cpdef np.ndarray get_grid_x(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_x(self._bmi, grid_id,
                                        &grid_x[0], size))
        return grid_x
    ...
    cpdef np.ndarray get_grid_y(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_y(self._bmi, grid_id,
                                        &grid_y[0], size))
        return grid_y
    ...
    cpdef np.ndarray get_grid_z(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_z(self._bmi, grid_id,
                                        &grid_z[0], size))
        return grid_z
    ...
    cpdef np.ndarray get_grid_connectivity(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_connectivity(self._bmi, grid_id,
                                                   &conn[0], size))
        return conn
    ...
    cpdef np.ndarray get_grid_offset(self, grid_id):
        ...
        ok_or_raise(<int>get_grid_offset(self._bmi, grid_id,
                                             &offset[0], size))
        return offset

    cpdef object get_var_type(self, var_name):
        self.reset_str_buffer()
        ok_or_raise(<int>get_var_type(self._bmi,
                                          to_bytes(var_name),
                                          len(var_name),
                                          self.STR_BUFFER,
                                          MAX_TYPE_NAME))
    ...
    cpdef object get_var_units(self, var_name):
        self.reset_str_buffer()
        ok_or_raise(<int>get_var_units(self._bmi,
                                           to_bytes(var_name),
                                           len(var_name),
                                           self.STR_BUFFER,
                                           MAX_UNITS_NAME))
    ...
    cpdef int get_var_itemsize(self, var_name):
        cdef int itemsize
        ok_or_raise(<int>get_var_itemsize(self._bmi,
                                              to_bytes(var_name),
    ...
    cpdef int get_var_nbytes(self, var_name):
        cdef int nbytes
        ok_or_raise(<int>get_var_nbytes(self._bmi,
                                            to_bytes(var_name),
                                            len(var_name), &nbytes))
        return nbytes
    ...
    cpdef np.ndarray get_value_int(self, var_name, grid_size):
        ...
        ok_or_raise(<int>get_value_int(self._bmi,
                                           to_bytes(var_name),
    ...
    cpdef np.ndarray get_value_float(self, var_name, grid_size):
        ...
        ok_or_raise(<int>get_value_float(self._bmi,
                                             to_bytes(var_name),
                                             len(var_name),
    ...
    cpdef np.ndarray get_value_double(self, var_name, grid_size):
        ...
        ok_or_raise(<int>get_value_double(self._bmi,
                                              to_bytes(var_name),
                                              len(var_name),
                                              buffer.data, grid_size))
        return buffer
    ...
    cpdef np.ndarray get_value_ref(self, var_name):
        ...
        ok_or_raise(<int>get_value_ref(self._bmi, to_bytes(var_name),
                                      len(var_name), &ptr))
    ...
    cpdef set_value(self, var_name, np.ndarray buffer):
        ...
        if type.startswith('double'):
            ok_or_raise(<int>set_value_double(self._bmi,
                                                  to_bytes(var_name),
                                                  len(var_name),
                                                  buffer.data,
                                                  grid_size))
        elif type.startswith('int'):
            ok_or_raise(<int>set_value_int(self._bmi,
                                               to_bytes(var_name),
                                               len(var_name),
                                               buffer.data,
                                               grid_size))
        else:
            ok_or_raise(<int>set_value_float(self._bmi,
                                                 to_bytes(var_name),
                                                 len(var_name),
                                                 buffer.data,
                                                 grid_size))
